...
async function checkArguments(fn, request, reply) {
    ...
    if (!user) { 
        reply.code(403).send(replyBuilder(false, { "message": "User not exists" }));
        return false;
    }

    if (!uploadValue) {
        reply.code(422).send(replyBuilder(false, { "message": "You have not uploaded a file" }));
        return false;
    }

    const fileType = await fromBuffer(uploadValue);
    if (!/(image)/.test(fileType.mime)) {
        reply.code(422).send(replyBuilder(false, { "message": "File must be an image" }));
        return false;
    }

    if (body.type === "cover") {
        if (!body.bookID) {
            reply.code(422).send(replyBuilder(false, { "message": "You have not specified a book ID" }));
            return false;
        }

        if (!await Book.checkOwnership(body.bookID, user["_id"])) {
            reply.code(403).send(replyBuilder(false, { "message": "You have no permossion to upload cover for this book" }));
            return false;
        }
    }
